...
class BatchManager(QObject):
    ...
    def move_file(self, from_index: int, to_index: int) -> bool:
        ...
        try:
            if (
                0 <= from_index < len(self._file_queue)
                and 0 <= to_index < len(self._file_queue)
        ...
        try:
            if 0 <= index < len(self._file_queue):
                ...
                duplicate_item = BatchFileItem(
                    str(original_item.file_path), f"{original_item.display_name} (Copy)"
                )
    ...
    def mark_file_processing_started(self, index: int) -> None:
        ...
        if 0 <= index < len(self._file_queue):
            ...
            self.file_processing_started.emit(
                index, self._file_queue[index].display_name
            )

    def mark_file_processing_completed(
        self, index: int, output_files: List[str]
    ) -> None:
        ...
        if 0 <= index < len(self._file_queue):
            ...
            self.file_processing_completed.emit(
                index, self._file_queue[index].display_name, output_files
            )
    ...
    def mark_file_processing_error(self, index: int, error_message: str) -> None:
        ...
        if 0 <= index < len(self._file_queue):
            ...
            self.file_processing_error.emit(
                index, self._file_queue[index].display_name, error_message
            )
