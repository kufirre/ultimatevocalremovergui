...
class BatchProcessingWorker(QObject):
    ...
    def _process_next_file(self):
        ...
        try:
            ...
            logger.error(f"Error starting processing for file {self._current_file_index}: {e}")
    ...
    def _start_file_processing(self, settings: Dict):
        ...
        try:
            ...
            # Connect signals
            self._processing_thread.progress_updated.connect(self._on_file_progress_updated)
            self._processing_thread.processing_finished.connect(self._on_file_processing_finished)
    ...
    def _on_file_progress_updated(self, progress: int, message: str):
        ...
        overall_progress = ((files_completed + current_file_progress) / self._total_files) * 100
        overall_progress = min(int(overall_progress), 99)  # Cap at 99% until truly complete
    ...
    def _on_file_processing_finished(self, success: bool, message: str):
        ...
        if success:
            ...
            logger.info(f"Completed processing file {self._current_file_index + 1}: {current_file.display_name}")
        else:
            # Mark file as error
    ...
    def _handle_file_error(self, error_message: str):
        ...
        logger.error(f"Error processing file {self._current_file_index + 1} ({current_file.display_name}): {error_message}")
    ...
    def _finish_batch(self):
        ...
        self.overall_progress.emit(100, f"Batch completed: {stats['completed']} successful, {stats['error']} errors")
        
        logger.info(f"Batch processing finished: {stats['completed']} successful, {stats['error']} errors")
