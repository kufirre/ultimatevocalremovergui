...
for path in MODEL_TYPE_PATHS.values():
    if path: path.mkdir(parents=True, exist_ok=True) # Check if path is not None
...
def fetch_online_model_catalog() -> Dict[str, Any]:
    ...
    if ONLINE_CATALOG_CACHE_FILE.exists():
        try:
            ...
            with open(ONLINE_CATALOG_CACHE_FILE, 'r', encoding='utf-8') as f:
...
def download_model_file(model_name: str, download_url: str, model_type: str, 
    ...
    try:
        for url, local_path, file_type_name in files_to_download:
            if progress_callback: progress_callback(Path(url).name, 0)
            
            print(f"Downloading {file_type_name} {Path(url).name} from {url} to {local_path}...")
        ...
        for _, local_p, _ in files_to_download:
            if local_p.exists(): local_p.unlink(missing_ok=True)
...
if __name__ == '__main__':
    ...
    if catalog:
        ...
        for key, value in list(catalog.items())[:2]: 
            if isinstance(value, dict): print(f"  {key}: {list(value.keys())[:3]}...") 
