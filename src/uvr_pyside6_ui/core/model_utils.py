...
def get_models_dir_paths():
    ...
    return {
        ...
        ac.MDX_ARCH_TYPE: models_dir_path / ac.MODEL_TYPE_SUBDIRS[ac.MDX_NET_MODELS_KEY],
        ac.DEMUCS_ARCH_TYPE: models_dir_path / ac.MODEL_TYPE_SUBDIRS[ac.DEMUCS_MODELS_KEY],
    }
...
def scan_models_directory(model_type: str) -> List[str]:
    ...
    extensions_map = {
        ...
        ac.DEMUCS_ARCH_TYPE: ac.DEMUCS_LEGACY_SCAN_EXTENSIONS + ac.DEMUCS_V3_V4_SCAN_EXTENSIONS,
    }
    ...
    try:
        ...
        for identifier in scanned_identifiers:
            display_name, was_mapped = _get_display_name_from_mapper(identifier, name_mapper)
...
def _scan_path_for_identifiers(
    ...
    excluded_filenames = ["model_data", "model_name_mapper", "download_links", "mixer", "mixer_val"]
...
def _load_name_mapper(model_dir: Path) -> dict:
    ...
    if mapper_file_path.is_file():
        try:
            ...
            logger.error(f"JSONDecodeError when loading name mapper from {mapper_file_path}: {e}")
        except Exception as e:
            logger.error(f"Unexpected error loading name mapper from {mapper_file_path}: {e}")
    else:
        logger.debug(f"Name mapper file not found at: {mapper_file_path}")
    return {}


def _get_display_name_from_mapper(scanned_identifier: str, name_mapper: dict) -> tuple[str, bool]:
...
def _strip_model_extension(model_name: str) -> str:
    ...
    extensions_to_strip = [".pth", ".onnx", ".ckpt", ".pt", ".bin", ".pkl", ".yaml", ".th", ".gz"]
...
def determine_model_process_method(model_name: str) -> Optional[str]:
    ...
    logger.warning(f"Warning: Could not determine process method for model: {model_name}")
    return None
...
def get_karaoke_models() -> List[str]:
    ...
    try:
        # Import ModelData here to avoid circular dependency
        ...
        minimal_settings = {
            ...
            ac.SettingKeys.WAV_TYPE_SET: 'PCM_16',
        ...
        for model_name in vr_models:
            try:
                ...
                if model_data.model_status and (model_data.is_karaoke or model_data.is_bv_model):
                    karaoke_models.append(model_name)
                    
            except Exception as model_error:
                logger.debug(f"Could not check karaoke metadata for VR model {model_name}: {model_error}")
                continue
        ...
        for model_name in mdx_models:
            try:
                ...
                logger.debug(f"Could not check karaoke metadata for MDX model {model_name}: {model_error}")
                continue
