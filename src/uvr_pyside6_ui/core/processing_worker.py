...
class ProcessingWorker(QObject):
    ...
    def _process_vr_arch_direct(self, input_audio: np.ndarray) -> bool:
        ...
        with temporary_audio_file(
            input_audio
        ) as temp_audio_file, gpu_memory_management():
            ...
            # Context manager handles temp file cleanup automatically
            return True
    ...
    def _process_demucs_direct(self, input_audio: np.ndarray) -> bool:
        ...
        with temporary_audio_file(input_audio) as temp_audio_file:
            self._write_to_console(
                f"  ✓ Created temp audio file: {temp_audio_file}", ""
            )
        ...
        try:
            ...
            if "demucs_model" in locals():
                ...
                if hasattr(torch.cuda, "empty_cache"):
                    torch.cuda.empty_cache()
            # Temporary file cleanup is handled by context manager

        return True

    def _save_temp_audio(self, audio: np.ndarray) -> str:
        """Save audio array to a temporary file - DEPRECATED: Use temporary_audio_file context manager instead"""
        logger.warning(
            "_save_temp_audio is deprecated, use temporary_audio_file context manager instead"
        )
        try:
            with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as f:
                # Ensure audio is in correct format for saving
                if audio.ndim == 1:
                    # Mono audio
                    sf.write(f.name, audio, ac.DEFAULT_SAMPLE_RATE)
                elif audio.ndim == 2:
                    if audio.shape[0] == 2:
                        # (2, N) format - transpose to (N, 2) for soundfile
                        sf.write(f.name, audio.T, ac.DEFAULT_SAMPLE_RATE)
                    else:
                        # (N, 2) format - use as is
                        sf.write(f.name, audio, ac.DEFAULT_SAMPLE_RATE)
                return f.name
