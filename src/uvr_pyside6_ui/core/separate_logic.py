...
def write_audio_logic(
    ...
    base_text_console = (
        process_data.get("base_text_console", "") if process_data else ""
    )
    if console_logger:
        console_logger(
            f"{ac.SAVING_STEM_MESSAGE[0]}{stem_name or 'stem'}{ac.SAVING_STEM_MESSAGE[1]}",
            base_text=base_text_console,
        )
    ...
    if model_data.save_format != ac.WAV:
        if pydub:
            try:
                ...
                target_format_path = stem_path.with_suffix(
                    f".{model_data.save_format.lower()}"
                )
                if model_data.save_format == ac.FLAC:
                    audio_segment.export(str(target_format_path), format="flac")
                elif model_data.save_format == ac.MP3:
                    audio_segment.export(
                        str(target_format_path),
                        format="mp3",
                        bitrate=model_data.mp3_bit_set,
                    )
                ...
                logger.error(
                    f"Error converting {stem_path} to {model_data.save_format}: {e}"
                )
                if console_logger:
                    console_logger(
                        f"Warning: Could not convert to {model_data.save_format}, WAV saved.",
                        base_text=base_text_console,
                    )
        elif console_logger:
            console_logger(
                f"Warning: pydub not found. Cannot convert to {model_data.save_format}. WAV saved.",
                base_text=base_text_console,
            )
...
class SeparatorAttributesLogic:
    def __init__(self, model_data: ModelData, process_data: Dict[str, Any]):
        ...
        self.set_progress_bar = process_data.get(
            "set_progress_bar", lambda b, a=0: None
        )
        self.write_to_console = process_data.get(
            "write_to_console", lambda m, b="": None
        )
        self.base_text_console = process_data.get("base_text_console", "")
        self.process_iteration_callback = process_data.get(
            "process_iteration", lambda: None
        )
        self._is_running_check = process_data.get("_is_running_check", lambda: True)

        self.audio_file_path = (
            Path(model_data.audio_file) if model_data.audio_file else None
        )
        self.export_path = (
            Path(model_data.export_path) if model_data.export_path else None
        )
        self.audio_file_base = (
            self.audio_file_path.stem if self.audio_file_path else "output"
        )
        ...
        if model_data.is_gpu_conversion:
            if MPS_AVAILABLE:
                ...
                self.device = torch.device(
                    f"cuda:{model_data.device_set}"
                    if model_data.device_set != ac.DEFAULT
                    else "cuda"
                )
        self.run_type = [ac.CPU_EXECUTION_PROVIDER]
        if self.device.type == "cuda":
            self.run_type = [ac.CUDA_EXECUTION_PROVIDER]
        ...
        self.cached_source_callback = process_data.get(
            "cached_source_callback", lambda pm, mn: (None, None)
        )
        self.cached_model_source_holder = process_data.get(
            "cached_model_source_holder", lambda pm, s, mn: None
        )
    ...
    def _console_log_base(self, message: str):
        self.write_to_console(message, base_text=self.base_text_console)

    def _console_log(self, message: str):
        self.write_to_console(message, base_text="")
    ...
    def _prepare_mix(self) -> Optional[np.ndarray]:
        ...
        if input_audio_array is not None:
            ...
            if (
                input_audio_array.shape[0] < input_audio_array.shape[1]
                and input_audio_array.ndim == 2
            ):  # (channels, length)
            ...
            logger.info(
                f"Loading audio file: {self.audio_file_path.name} for {self.md.model_basename}..."
            )
            ...
            logger.error(
                f"Error: No audio input (file or array) for {self.md.model_basename}."
            )
            return None

    def _write_stem(self, stem_name: str, stem_data: np.ndarray, samplerate: int):
        if not self.export_path:
            logger.error("Error: Export path not set.")
            return
        ...
        stem_filename = (
            "_".join(filter(None, filename_parts)) + f".{self.md.save_format.lower()}"
        )
        stem_save_path = str(self.export_path / stem_filename)
        write_audio_logic(
            stem_save_path, stem_data, samplerate, self.md, stem_name, self.process_data
        )
    ...
    def _pitch_fix(
        ...
        if not spec_utils:
            return source_audio
        if source_audio.shape[0] > source_audio.shape[1]:
            source_audio = source_audio.T
        changed_pitch_audio = spec_utils.change_pitch_semitones(
            source_audio, sr_pitched, semitone_shift=self.md.semitone_shift
        )[0]
        if org_mix_shape_ref.shape[0] > org_mix_shape_ref.shape[1]:
            org_mix_shape_ref = org_mix_shape_ref.T
