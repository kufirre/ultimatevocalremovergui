...
class BatchFilePresenter(QObject):
    ...
    def _setup_connections(self):
        ...
        self.batch_manager.file_processing_started.connect(
            self._on_file_processing_started
        )
        self.batch_manager.file_processing_completed.connect(
    ...
    def handle_add_files(self):
        ...
        if file_paths:
            ...
            if added_count > 0:
                ...
                QTimer.singleShot(
                    3000,
                    lambda: self.view.set_info_text(
    ...
    def _update_batch_ready_state(self):
        ...
        is_ready = (
            self.batch_manager.get_queue_size() > 0
            and not self.batch_manager.is_processing()
        )
    ...
    def _on_batch_completed(self, results: list):
        ...
        if errors == 0:
            self.view.set_info_text(
                f"Batch completed successfully: {completed} files processed"
            )
        else:
            self.view.set_info_text(
                f"Batch completed: {completed} successful, {errors} errors"
            )
        ...
        self._update_batch_ready_state()
        logger.info(
            f"Batch processing completed: {completed} successful, {errors} errors"
        )
    ...
    @Slot(int, str, list)
    def _on_file_processing_completed(
        self, index: int, filename: str, output_files: list
    ):
