...
class EnsembleSimplePresenter(QObject):
    ...
    def __init__(self, view, adapter: UVRCoreAdapter, settings_dialog_presenter=None):
        ...
        # All advanced settings are delegated to the advanced dialog
        self._advanced_settings: dict = {
    ...
    def on_advanced_settings_requested(self):
        ...
        if self.settings_dialog_presenter:
            ...
            current_settings = self.get_current_ui_state()
            ...
            dialog.settings_updated.connect(self.update_from_advanced_dialog)
    ...
    @Slot(dict)
    def update_from_advanced_dialog(self, settings: dict):
        """Update simple presenter state from advanced dialog settings."""
        self._current_main_stem_pair = settings.get(
            "ensemble_main_stem_pair", self._current_main_stem_pair
        )

        # Store the desired algorithm before updating options
        desired_algorithm = settings.get("ensemble_algorithm", self._current_algorithm)

        self._currently_selected_models = settings.get("ensemble_selected_models", [])

        # Update advanced settings
        self._advanced_settings.update(
            {
                "save_all_outputs": settings.get("save_all_outputs", True),
                "append_ensemble_name": settings.get("append_ensemble_name", False),
                "use_waveform_ensemble": settings.get("use_waveform_ensemble", False),
            }
        )
        ...
        # Update algorithm options first
        self._update_algorithm_options()

        # Then set the desired algorithm if it's valid for the current stem pair
        if self._current_main_stem_pair == "4 Stem Ensemble":
            valid_algorithms = ac.ENSEMBLE_ALGORITHM_4_STEM_OPTIONS
        else:
            valid_algorithms = ac.ENSEMBLE_ALGORITHM_OPTIONS

        if desired_algorithm in valid_algorithms:
            self._current_algorithm = desired_algorithm
    ...
    def get_current_ui_state(self) -> dict:
        ...
        return {
            ...
            "save_all_outputs": self._advanced_settings["save_all_outputs"],
            "append_ensemble_name": self._advanced_settings["append_ensemble_name"],
            "use_waveform_ensemble": self._advanced_settings["use_waveform_ensemble"],
        }

    def get_settings(self) -> dict:
        """Get current ensemble settings for processing.

        This method is required by the main application for ensemble processing.
        Returns the same data as get_current_ui_state() but with the expected method name.
        """
