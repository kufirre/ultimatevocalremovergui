...
class MDXNetSettingsPresenter(QObject):
    ...
    def __init__(self, view):
        ...
        print("MDXNetSettingsPresenter Initialized.")

    @Slot(int)
    def set_segment_size(self, value):
        self._segment_size = value
        print(f"Presenter (MDX): Segment Size = {value}")

    @Slot(float)
    def set_overlap(self, value):
        self._overlap = value
        print(f"Presenter (MDX): Overlap = {value:.2f}")

    @Slot(str)
    def set_match_method(self, value):
        self._match_method = value
        print(f"Presenter (MDX): Match Method = {value}")
