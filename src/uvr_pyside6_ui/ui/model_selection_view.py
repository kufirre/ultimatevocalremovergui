...
class ModelSelectionView(QWidget):
    ...
    # This signal will be emitted with the text of the user-selected model
    ...
    def __init__(self, parent=None):
        ...
        # Process Method ComboBox
        method_layout = QHBoxLayout()
        method_label = QLabel("Process Method:")
        self.method_combo = QComboBox()
        # currentTextChanged is fine for method_combo as its changes are less complex
        ...
        # CORRECTED SIGNAL CONNECTION: Use activated[int]
    ...
    # NEW SLOT to handle activated[int] signal from QComboBox
    @Slot(int)
    def _handle_model_combo_activated_by_user(self, index: int):
        """
        Handles the QComboBox.activated[int] signal.
        Gets the text of the selected item and emits the model_selected_by_user signal.
        """
    ...
    def set_process_methods(self, methods: List[str]):
        ...
        if methods:
            ...
            if self.method_combo.count() > 0:  # Ensure items were added
        ...
        if first_method_text or not methods:
            ...
            self.process_method_changed.emit(first_method_text)

    @Slot(list)
    def set_models(self, models: List[str]) -> str:
        """
        Populates the Model combobox and adds the download option.
        Programmatically sets selection to the first actual model or download option.
        Returns the text of the item that was programmatically selected.
        """
        ...
        if has_actual_models:
            ...
            self.model_combo.setCurrentIndex(0)  # This selects "Download More..."
    ...
    def set_current_model_text(self, model_text_to_select: str):
        """Programmatically sets the current displayed model in the combobox."""
        self.model_combo.blockSignals(True)
        idx = -1
        ...
        if idx != -1:
            ...
            # Fallback if the model_text_to_select isn't found (e.g., list just repopulated)
            # Try to select the first actual model, if any.
            first_item = self.model_combo_model.item(0)
            if first_item and first_item.text() != DOWNLOAD_MORE_MODELS_TEXT:
                self.model_combo.setCurrentIndex(0)
            # If only "Download More..." is there, it will be at index 0
    ...
    # ... (add_settings_panel, show_settings_panel, set_ensemble_checked remain the same) ...
