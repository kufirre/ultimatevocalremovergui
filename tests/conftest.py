...
def mock_separator_logic():
    ...
    with patch('uvr_pyside6_ui.core.processing_worker.SeperateVRLogic', return_value=mock_vr_separator), \
         patch('uvr_pyside6_ui.core.processing_worker.SeperateMDXLogic', return_value=mock_mdx_separator), \
         patch('uvr_pyside6_ui.core.processing_worker.SeperateMDXCLogic', return_value=mock_mdx_separator), \
         patch('uvr_pyside6_ui.core.processing_worker.SeperateDemucsLogic', return_value=mock_demucs_separator), \
         patch('uvr_pyside6_ui.core.processing_worker.prepare_mix_logic') as mock_prepare_mix, \
         patch('uvr_pyside6_ui.core.processing_worker.write_audio_logic') as mock_write_audio, \
         patch('uvr_pyside6_ui.core.processing_worker.clear_gpu_cache_logic') as mock_clear_gpu:
...
def assert_audio_array_valid(audio_array, expected_shape=None, expected_dtype=np.float32):
    ...
    assert audio_array.dtype == expected_dtype, f"Expected dtype {expected_dtype}, got {audio_array.dtype}"
    
    if expected_shape:
        assert audio_array.shape == expected_shape, f"Expected shape {expected_shape}, got {audio_array.shape}"
...
def mock_uvr_core_adapter():
    ...
    adapter.get_available_models.return_value = ["model1.pth", "model2.onnx", "model3.yaml"]
