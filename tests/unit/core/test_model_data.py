...
class TestModelData:
        ...
        with patch.object(ModelData, '_determine_model_path') as mock_path:
            with patch.object(ModelData, '_get_model_hash') as mock_hash:
                with patch.object(ModelData, '_load_and_derive_model_properties'):
                    ...
                    assert model_data.vr_voc_inst_secondary_model == f'{ac.VR_ARCH_TYPE}==secondary_model.pth'
    ...
    def test_determine_model_path_vr_model(self):
        ...
        model_data = ModelData(
            ...
            model_name="test_vr_model"
        )
        
        with patch('uvr_pyside6_ui.core.model_data.VR_MODELS_DIR_PATH') as mock_vr_dir:
            mock_vr_dir.__truediv__ = Mock(return_value=Path("/models/VR_Models/test_vr_model.pth"))
    ...
    def test_determine_model_path_mdx_with_mapper(self):
        ...
        model_data = ModelData(
            ...
            model_name="Beautiful Display Name"
        )
        ...
        with patch('uvr_pyside6_ui.core.model_data.MDX_HASH_DIR_PATH') as mock_hash_dir:
            ...
            with patch.object(Path, 'exists', return_value=True):
                with patch('builtins.open', mock_open(read_data=json.dumps(mock_mapper))):
                    with patch('uvr_pyside6_ui.core.model_data.MDX_MODELS_DIR_PATH') as mock_mdx_dir:
    ...
    def test_determine_model_path_demucs_with_version_detection(self):
        ...
        with patch('uvr_pyside6_ui.core.model_data.DEMUCS_MODELS_DIR_PATH') as mock_demucs_dir:
            with patch('uvr_pyside6_ui.core.model_data.DEMUCS_NEWER_REPO_DIR_PATH') as mock_newer_dir:
    ...
    def test_load_and_derive_model_properties_vr(self):
        ...
        with patch('uvr_pyside6_ui.core.model_data.VR_HASH_DIR_PATH') as mock_hash_dir:
            ...
            with patch.object(hash_file, 'exists', return_value=True):
                with patch('builtins.open', mock_open(read_data=json.dumps(mock_params))):
                    with patch('uvr_pyside6_ui.core.model_data.VR_PARAM_DIR_PATH') as mock_param_dir:
                        param_file = mock_param_dir / "test_param.json"
                        with patch.object(param_file, 'exists', return_value=True):
                            with patch('uvr_pyside6_ui.core.model_data.ModelParameters') as mock_model_params:
    ...
    def test_load_and_derive_model_properties_mdx_c(self):
        ...
        with patch('uvr_pyside6_ui.core.model_data.MDX_HASH_DIR_PATH') as mock_hash_dir:
            with patch('uvr_pyside6_ui.core.model_data.MDX_C_CONFIG_PATH_DIR') as mock_config_dir:
    ...
    def test_load_ensemble_config_success(self):
        ...
        model_data = ModelData(
            model_path="/path/to/ensemble.json",
        ...
        with patch.object(Path, 'exists', return_value=True):
            with patch('builtins.open', mock_open(read_data=json.dumps(mock_ensemble_data))):
    ...
    def test_load_ensemble_config_file_not_found(self):
        ...
        model_data = ModelData(
            model_path="/nonexistent/ensemble.json",
            is_ensemble_mode=True
        )
    ...
    def test_create_live_ensemble_from_ui_settings(self):
        ...
        with patch.object(model_data, '_determine_model_process_method') as mock_determine:
            with patch.object(ModelData, 'from_settings_dict') as mock_from_settings:
    ...
    def test_determine_model_process_method_by_extension(self):
        ...
        assert model_data._determine_model_process_method("model.pth") == ac.VR_ARCH_TYPE
        assert model_data._determine_model_process_method("model.onnx") == ac.MDX_ARCH_TYPE
        assert model_data._determine_model_process_method("model.ckpt") == ac.MDX_ARCH_TYPE
        assert model_data._determine_model_process_method("model.yaml") == ac.DEMUCS_ARCH_TYPE
        assert model_data._determine_model_process_method("model.th") == ac.DEMUCS_ARCH_TYPE
    ...
    def test_get_secondary_model_settings_vocal_inst(self):
        ...
        assert model_data.vr_voc_inst_secondary_model == f"{ac.VR_ARCH_TYPE}==secondary.pth"
        assert model_data.vr_voc_inst_secondary_model_scale == 0.8
        # Test that the method ran without error
        assert model_data.process_method == ac.VR_ARCH_TYPE
    ...
    def test_parse_stem_pair_to_primary_stem(self):
        ...
        assert model_data._parse_stem_pair_to_primary_stem("Vocals/Instrumental") == ac.VOCAL_STEM
        assert model_data._parse_stem_pair_to_primary_stem("Other/No Other") == ac.OTHER_STEM
        assert model_data._parse_stem_pair_to_primary_stem("Drums/No Drums") == ac.DRUM_STEM
        assert model_data._parse_stem_pair_to_primary_stem("Bass/No Bass") == ac.BASS_STEM
        assert model_data._parse_stem_pair_to_primary_stem("Unknown") == ac.VOCAL_STEM  # Default

    def test_map_ui_algorithm_to_ensemble_type(self):
        """Test mapping UI algorithm to ensemble type."""
        model_data = ModelData()
        
        assert model_data._map_ui_algorithm_to_ensemble_type("Average") == ac.AVERAGE_ENSEMBLE
        assert model_data._map_ui_algorithm_to_ensemble_type("Max Spec") == ac.MAX_SPEC_ENSEMBLE
        assert model_data._map_ui_algorithm_to_ensemble_type("Min Spec") == ac.MIN_SPEC_ENSEMBLE
        assert model_data._map_ui_algorithm_to_ensemble_type("Unknown") == ac.AVERAGE_ENSEMBLE  # Default
    ...
    @pytest.mark.parametrize("process_method", [ac.VR_ARCH_TYPE, ac.MDX_ARCH_TYPE, ac.DEMUCS_ARCH_TYPE])
    ...
    @pytest.mark.parametrize("ensemble_algorithm", [ac.AVERAGE_ENSEMBLE, ac.MAX_SPEC_ENSEMBLE, ac.MIN_SPEC_ENSEMBLE])
...
class TestGetProjectRoot:
    ...
    def test_get_project_root_fallback(self):
        ...
        with patch.object(Path, 'resolve') as mock_resolve:
            ...
            with patch.object(Path, 'cwd', return_value=Path("/fallback/dir")) as mock_cwd:
                result = get_project_root()
