...
class TestUVRCoreAdapter:
        ...
        with patch.object(Path, 'resolve') as mock_resolve:
            with patch.object(Path, 'cwd', return_value=self.temp_dir):
                ...
                def mock_is_dir(path_instance):
                    ...
                    return path_str.endswith('models') and path_str.startswith(temp_dir_str)
    ...
    def test_get_locally_installed_primary_model_filenames_vr(self):
        ...
        with patch.object(adapter, '_get_project_models_dir', return_value=self.models_dir):
            result = adapter._get_locally_installed_primary_model_filenames(ac.VR_ARCH_MODELS_KEY)
    ...
    def test_get_locally_installed_primary_model_filenames_mdx(self):
        ...
        with patch.object(adapter, '_get_project_models_dir', return_value=self.models_dir):
            result = adapter._get_locally_installed_primary_model_filenames(ac.MDX_NET_MODELS_KEY)
    ...
    def test_get_downloadable_models_for_type_success(self):
        ...
        with patch.object(adapter, 'get_online_catalog', return_value=mock_catalog):
            with patch.object(adapter, '_get_locally_installed_primary_model_filenames', 
                             return_value={"installed.pth"}):
                with patch.object(adapter, '_get_primary_filename_from_download_info') as mock_get_filename:
                    # Mock the filename extraction to return the expected filenames
                    mock_get_filename.side_effect = lambda x: x if isinstance(x, str) else None
    ...
    def test_get_model_info_demucs_model(self):
        ...
        assert result is not None
        assert result['demucs_stems'] == [ac.VOCAL_STEM, ac.DRUM_STEM, ac.BASS_STEM, ac.OTHER_STEM]
        assert result['is_4_stem'] is True
    ...
    def test_get_model_info_demucs_2stem_model(self):
        ...
        with patch.object(adapter, '_get_project_models_dir', return_value=self.models_dir):
            result = adapter.get_model_info("test_2stem_vocal_model", ac.DEMUCS_MODELS_KEY)
    ...
    def test_get_model_info_exception_handling(self):
        ...
        with patch.object(adapter, '_get_project_models_dir', side_effect=Exception("Test error")):
    ...
    def test_get_available_methods(self):
        ...
        expected = [ac.VR_ARCH_MODELS_KEY, ac.MDX_NET_MODELS_KEY, ac.DEMUCS_MODELS_KEY, ac.ENSEMBLE_MODELS_KEY]
        assert result == expected
    ...
    def test_get_available_models_vr(self):
        ...
        with patch.object(adapter, '_get_project_models_dir', return_value=self.models_dir):
            ...
            with patch.object(adapter, '_get_display_name_from_mapper', side_effect=mock_get_display_name):
...
class TestUVRCoreAdapterIntegration:
    ...
    def test_download_workflow_integration(self, mock_fetch):
        ...
        mock_catalog = {
            ac.ONLINE_VR_DOWNLOAD_LIST_KEY: {
                "Test Online Model": "test_model.pth"
            }
        }
        mock_fetch.return_value = mock_catalog
        ...
        with patch.object(adapter, '_get_locally_installed_primary_model_filenames', 
            ...
            downloadable = adapter.get_downloadable_models_for_type(ac.VR_ARCH_MODELS_KEY)
